...
export const showMessage = (type, msg, ...time) => {
	...
	const msgDiv = document.querySelector('#message-reg');
...
export const showMessageStatic = (type, msg, ...time) => {
	clearTimeout(msgTimeout.value);
	hideMessage();
	let color;
	let bgcolor;
	...
	const msgDiv = document.querySelector('#message-static');
	const msgInner = msgDiv.querySelector('.message');
	msgInner.innerHTML = msg;
	msgInner.style = `color:${color};background-color:${bgcolor};opacity:1;`;
	msgDiv.classList.remove('d-none');
	msgTimeout.value = setTimeout(hideMessage, duration);
};
...
export const hideMessage = () => {
	...
	const msgStatic = document.querySelector('#message-static');
	msgStatic.classList.add('d-none');
